...
let apiUsers = "http://localhost:8001/users";
...
btnReg.addEventListener("click", () => {
  ...
  if ((user.login === "elzar") & (user.password === "bekbol")) {
    user.isAdmin = true;
  }
  fetch(apiUsers, {
    method: "POST",
...
btnAuth.addEventListener("click", () => {
  ...
  fetch(apiUsers)
    ...
    .then((data) => {
      ...
      if (userCheck) {
        localStorage.setItem("users", JSON.stringify([userName, adminRole]));
        isLogin = true;
        authModal.style.display = "none";
        readProducts();
      } else {
      ...
      if (logUser.isAdmin) {
        let adminCheck = JSON.parse(localStorage.getItem("users"));
        adminRole = true;
...
function readProducts() {
  fetch(
    `${api}?q=${searchValue}&_page=${currentPage}&_limit=5&productCategory_like=${category}`
  )
    .then((res) => res.json())
    .then((data) => {
      ...
      data.forEach((item) => {
        ...
        <div class="card m-3 shadow p-1 mb-5 bg-white rounded" style="width: 14rem; cursor: pointer">
          ...
          <div class="card-body" style="display: flex; flex-direction: column; justify-content: space-between">
            ...
            <div>
            <h6 class="card-price" style = "color: #ba1010">
            ${item.productPrice} KGS
            </h6>
            ${
              adminRole
                ? `<div style="display: flex; justify-content: center; justify-content: space-evenly">
            <button class="btn mt-1 btnDelete" style="background-color: #ba1010; color: white; display: flex" id="${item.id}">Delete</button>
            <button class="btn mt-1 btnEdit" style="background-color: #ba1010; color: white; display: flex" id="${item.id}" data-bs-toggle="modal" data-bs-target="#exampleModal">Edit</button>
            </div>`
                : ` `
            }
            ${
              isLogin
                ? `<button class="btn mt-1 btnCart" style="width: 100%; text-align: center; background-color: #ba1010; display: block;" id="${item.id}"><a href = "https://web.telegram.org/k/#@hareelz" target = "_blank" style = "text-decoration: none; color: white">Купить</a></button>`
                : ` `
            }
  ...
  try {
    fetch(`${api}/${id}`)
      ...
      .then((data) => {
        ...
        <div style="width: 35% background: url('https://wallpapercave.com/wp/wp6222694.png')">
        <img src="${data.productImage}" alt="" style = "max-width: 90%" />
      </div>
      <div style="width: 60%; height: 25rem; display: flex; flex-direction: column; justify-content: space-between">
        <div>
        <h6 class='title-ditalis'>${data.productName}</h6>
        <p style = "font-size: 40px">${data.productDetails}</p>
        </div>
        <div>
